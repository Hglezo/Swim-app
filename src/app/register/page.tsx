...
export default function RegisterPage() {
  ...
  // Debounce the confirm password value
  const debouncedConfirmPassword = useDebounce(formData.confirmPassword, 1500);
  ...
  // Check password match when debounced confirm password changes
  useEffect(() => {
    if (debouncedConfirmPassword && formData.password !== debouncedConfirmPassword) {
      setErrors(prev => ({
        ...prev,
        confirmPassword: 'Passwords do not match'
      }));
    } else if (debouncedConfirmPassword && formData.password === debouncedConfirmPassword) {
      setErrors(prev => ({
        ...prev,
        confirmPassword: ''
      }));
    }
  }, [debouncedConfirmPassword, formData.password]);
